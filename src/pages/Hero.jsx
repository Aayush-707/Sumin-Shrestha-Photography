...
export default function Main() {
  ...
  const handleCopy = async () => {
    try {
      await navigator.clipboard.writeText("+977-9818123456");
      setCopied(true);
    ...
    <main className='mb-10'>
      ...
      <div className="flex justify-center items-center gap-10 md:mt-40 mt-20 2xl:px-80 xl:px-60 lg:px-40 md:px-20 px-6">
        ...
        >
            ...
            <button onClick={handleCopy} className="group border flex justify-center items-center gap-1 w-[140px] p-2 whitespace-nowrap hover:bg-[#212121] cursor-pointer transition duration-300 ease-in-out">
              <svg
              ...
              <span className="text-black text-sm md:text-[16px] group-hover:text-white">{copied ? "Copied!" : "Call now"}</span>
            </button>
        </div>
      </div>
      <Overview />
